...
<div class="attendance-list">
    ...
    <div class="month-nav">
        <a class="month-link" href="/admin/attendance/staff/{{ $user->id }}?month={{ $previousMonth }}">← 前月</a>
        <p class="month-current">{{ $currentMonthLabel }}</p>
        <a class="month-link" href="/admin/attendance/staff/{{ $user->id }}?month={{ $nextMonth }}">翌月 →</a>
    </div>
    ...
    <table class="attendance-table">
        ...
        <tr class="table-row">
            ...
            <td>
                @if($day['id'])
                <a class="link-detail" href="/admin/attendance/{{ $day['id'] }}">詳細</a>
                @else
